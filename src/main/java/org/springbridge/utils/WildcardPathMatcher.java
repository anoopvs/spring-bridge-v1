 ...
 * isolation taken from Apache Cocoon.PathMatcher(WildcardPathMatcher) is
 * directly taken from Struts 1.x F/W code.
 * 
 * Spring UriTemplate style path and patterns are more appropriate for Spring
 * based projects, if projects are only using path parameter style patterns.We
 * need to switch it fully to AntPathMatcher for pattern matching and
 * UriTemplate for template variable substitution.
 *
...
public class WildcardPathMatcher implements PathMatcher {
	 ...
	 * @return The encoded string as an int array, terminated by the MATCH_END value
	 *         (don't consider the array length).
	 * @throws NullPointerException If data is null.
	 */
	public int[] compilePattern(@Nonnull final String pattern) {
		// Prepare the arrays
		int[] expr = new int[pattern.length() + 2];
			...
			for (int x = 1; x < buff.length; x++) {
				...
				if (slash) {
					...
					if (buff[x] == '\\') {
						...
						// If the previous character also was '*' match a path
	 ...
	 */
	public boolean match(@Nonnull final Map<String, String> map, @Nonnull final String data,
			@Nonnull final int[] expr) {

		Objects.requireNonNull(map, "No 'map' parameter provided");
		Objects.requireNonNull(data, "No 'data' parameter provided");
		Objects.requireNonNull(expr, "No 'pattern' parameter expression provided");
		...
		// We want the complete data for key "0"
		...
		// The expression character (MATCH_*)
	 ...
	 * This method return the index in d of the first occurrence after dpos of that
